...
import { ApiResponse, FailureResponse, SuccessResponse } from 'shared/interfaces/http.interface';
import { Roll, RollInput } from 'shared/models/roll';

export async function saveActiveRoll(roll: RollInput): Promise<ApiResponse<SuccessResponse | FailureResponse>> {
  try {
    ...
    const rollsToSave =
      rollsInStorage !== undefined ? [...rollsInStorage, createRoll(newRollId, roll)] : [createRoll(newRollId, roll)];
